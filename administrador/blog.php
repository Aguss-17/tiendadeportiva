...
// Procesamiento de imagen usando el nuevo handler
...
switch ($txtAccion) {
    case "Agregar":
        ...
        $sentenciaSQL = $conexion->prepare("INSERT INTO posts (titulo, contenido, meta_description, imagen, autor, categoria, estado, destacado) 
    ...
    case "Borrar":
        ...
        $sentenciaSQL->execute([':id' => $txtID]);
        $post = $sentenciaSQL->fetch(PDO::FETCH_LAZY);
        if (!empty($post['imagen']) && file_exists($directorioUploads . $post['imagen'])) @unlink($directorioUploads . $post['imagen']);
...
$sentenciaSQL = $conexion->prepare("SELECT * FROM posts ORDER BY fecha_publicacion DESC");
$sentenciaSQL->execute();
